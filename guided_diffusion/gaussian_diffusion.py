...
class GaussianDiffusion:
    ...
    def training_losses_segmentation(self, model, classifier, prior, posterior, x_start , t, model_kwargs=None, noise=None):
        if model_kwargs is None:
            model_kwargs = {}
        if noise is None:
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            ...
            if self.model_var_type in [
                ...
                B, C = x_t.shape[:2] #[B=10 batch, C channel ==1 ]
                C=1
            ...
            prior_latent_space = prior.forward(patch, model_output) 
            posterior_latent_space = posterior.forward(patch, mask)
            kl_div = kl.kl_divergence(posterior_latent_space, prior_latent_space)            
            terms["mse"] = mean_flat((target - model_output) ** 2) #[10,1,128,128]
            terms["kl"] = kl_div / 1000
            if "vb" in terms:
                terms["loss"] = terms["mse"] + terms["vb"] + terms["kl"]
            else:
                terms["loss"] = terms["mse"] + terms["kl"]
        else:
